    ...
    if (this.state.hasError) {
      return (
        <div className="w-full max-w-md mx-auto p-4">
          <div className="bg-gray-900/80 border-2 border-lime-700/50 rounded-2xl shadow-neon-green-light overflow-hidden backdrop-blur-md">
            <div className="p-8 text-center">
              <div className="w-20 h-20 bg-lime-900/50 rounded-2xl mx-auto mb-6 flex items-center justify-center border-2 border-lime-600/70 shadow-lg shadow-lime-500/30">
                <AlertTriangle className="w-10 h-10 text-lime-400" />
              </div>
              <h2 className="text-3xl font-bold text-lime-300 mb-2">Wallet Error</h2>
              <p className="text-lime-400/80 mb-8 text-base">
                {this.state.error?.message || 'An unexpected error occurred.'}
              </p>
              <button 
                onClick={() => window.location.reload()}
                className="px-6 py-3 bg-lime-900/50 border-2 border-lime-600/70 text-lime-300 rounded-xl hover:bg-lime-800/60 hover:border-lime-500 transition-colors"
              >
                Refresh Page
              </button>
            </div>
...
const TonWallet = () => {
  ...
  if (!connectedAddress) {
    return (
      <div className="w-full max-w-md mx-auto p-4 font-mono">
        <div className="bg-gray-900/80 border-2 border-lime-700/50 rounded-2xl shadow-neon-green-light overflow-hidden backdrop-blur-md">
          <div className="p-8 text-center">
            <div className="w-20 h-20 bg-lime-900/50 rounded-2xl mx-auto mb-6 flex items-center justify-center border-2 border-lime-600/70 shadow-lg shadow-lime-500/30">
              <Wallet className="w-10 h-10 text-lime-400" />
              </div>
            <h2 className="text-3xl font-bold text-lime-300 mb-2">Connect Your Wallet</h2>
            <p className="text-lime-400/80 mb-8 text-base">Connect your TON wallet to manage your digital assets</p>

            <div className="relative inline-block">
              <TonConnectButton className="relative !min-h-[52px] !px-8 !py-3 !bg-lime-600 !text-white !rounded-xl hover:!bg-lime-700 transition-colors" />
              </div>
                          </div>

          <div className="p-6 bg-gray-800/50 border-t border-lime-700/50">
            <h3 className="text-lg font-bold text-lime-300 mb-4 text-center">Wallet Features</h3>
                ...
                <div key={index} className="p-4 bg-gray-900/50 rounded-xl border border-lime-800/40 hover:border-lime-600/60 hover:bg-gray-800/70 transition-all duration-200">
                  <feature.icon className="w-6 h-6 text-lime-400 mb-2" />
                  <h4 className="text-sm font-bold text-lime-300 mb-1">{feature.title}</h4>
                  <p className="text-xs text-lime-400/80">{feature.desc}</p>
                      </div>
                    ))}
                  </div>
                  </div>
  ...
  return (
    <div className="w-full max-w-md mx-auto p-2 font-mono">
      <div className="bg-gray-900/80 border-2 border-lime-700/50 rounded-2xl shadow-neon-green-light overflow-hidden backdrop-blur-md">
        <div className="p-6 border-b border-lime-700/50">
          <div className="flex items-start justify-between">
                <div>
              <h2 className="text-base font-medium text-lime-400/80">TAPPs Wallet</h2>
              <div className="mt-3 text-5xl font-bold text-lime-300 tracking-tight">
                {formatBalance(`$${portfolioValue.toFixed(2)}`, hideBalances)}
                </div>
              <p className="mt-2 text-sm text-lime-500/80">{formatAddress(connectedAddressString)}</p>
              </div>
            <div className="flex items-center gap-2">
                <button 
                onClick={() => setHideBalances(!hideBalances)}
                className="p-2.5 bg-gray-800/50 hover:bg-gray-700/60 rounded-xl transition-colors border border-lime-800/40 text-lime-400"
                ...
                className="p-2.5 bg-gray-800/50 hover:bg.gray-700/60 rounded-xl transition-colors border border-lime-800/40 text-lime-400 disabled:opacity-50"
                title="Refresh"
              >
        ...
        <div className="px-4 py-4 border-b border-lime-700/50">
          <div className="grid grid-cols-4 gap-2">
            <button onClick={() => window.open('#', '_blank')} className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-blue-900/50 border border-blue-700/60 text-blue-300 hover:bg-blue-800/70 hover:border-blue-500/80 transition-all duration-200">
              <div className="w-8 h-8 rounded-lg bg-blue-800/40 flex items-center justify-center">
                <PlusCircle className="w-4 h-4 text-blue-300" />
                        </div>
              <span className="text-[10px] font-semibold">Buy</span>
                </button>
            <button onClick={() => setIsSendModalOpen(true)} className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-yellow-900/50 border border-yellow-700/60 text-yellow-300 hover:bg-yellow-800/70 hover:border-yellow-500/80 transition-all duration-200">
              <div className="w-8 h-8 rounded-lg bg-yellow-800/40 flex items-center justify-center">
                <ArrowUpRight className="w-4 h-4 text-yellow-300" />
              </div>
              <span className="text-[10px] font-semibold">Send</span>
              </button>
            <button onClick={() => setIsReceiveModalOpen(true)} className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-lime-900/50 border border-lime-700/60 text-lime-300 hover:bg-lime-800/70 hover:border-lime-500/80 transition-all duration-200">
              <div className="w-8 h-8 rounded-lg bg-lime-800/40 flex items-center justify-center">
                <ArrowDownLeft className="w-4 h-4 text-lime-300" />
            </div>
              <span className="text-[10px] font-semibold">Receive</span>
                        </button>
            <button onClick={() => window.open('https://dedust.io', '_blank')} className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-purple-900/50 border border-purple-700/60 text-purple-300 hover:bg-purple-800/70 hover:border-purple-500/80 transition-all duration-200">
              <div className="w-8 h-8 rounded-lg bg-purple-800/40 flex items-center justify-center">
        ...
        <div className="px-4 py-4 max-h-[40vh] overflow-y-auto">
          <h3 className="text-sm font-bold text-lime-300 mb-3 uppercase tracking-wider px-2">Assets</h3>
          <div className="space-y-2">
            <div className="flex items-center justify-between p-3 bg-gray-800/40 rounded-xl border border-lime-800/40 hover:bg-gray-700/50 transition-colors duration-200">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-blue-900/50 rounded-xl flex items-center justify-center border border-blue-700/60">
                  <Wallet className="w-5 h-5 text-blue-400" />
                                    ...
                                    <div>
                  <h4 className="text-lime-300 font-bold text-sm">TON</h4>
                  <p className="text-xs text-lime-500/80 font-medium">${(tonUsdPrice || 0).toFixed(2)}</p>
                                    </div>
                                  </div>
              <div className="text-right">
                <div className="text-lime-300 font-bold text-sm">
                  {isLoadingTON ? '...' : formatBalance(tonBalance, hideBalances)}
                                </div>
                <p className="text-xs text-lime-500/80 font-medium">${(parseFloat(tonBalance || '0') * (tonUsdPrice || 0)).toFixed(2)}</p>
                              </div>
                          </div>
            ...
            {isLoadingJettons ? (
              <div className="space-y-2">
                {[1,2,3].map((i) => (
                  <div key={i} className="p-3 bg-gray-800/40 rounded-xl border border-lime-800/40 animate-pulse flex items-center gap-3">
              ...
              allJettons.map((jetton) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 bg-gray-800/40 rounded-xl border border-lime-800/40 hover:bg-gray-700/50 transition-colors duration-200 text-left group"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-gray-700/40 rounded-xl flex items-center justify-center overflow-hidden border border-gray-600/50">
                        {enhancedJetton.jetton.image ? (
                          ...
                          <span className="text-lime-400 font-bold text-lg">{enhancedJetton.jetton.symbol?.[0] || '?'}</span>
                        )}
                      </div>
                      <div>
                        <div className="flex items-center gap-1.5">
                          <h4 className="text-lime-300 font-bold text-sm">{enhancedJetton.jetton.name}</h4>
                          {enhancedJetton.jetton.verified && (
                            <Shield className="w-3.5 h-3.5 text-blue-400" />
                                )}
                              </div>
                        <p className="text-xs text-lime-500/80 font-medium">
                          {enhancedJetton.jetton.verified ? 
                            `$${(registryData?.rateUsd || 0).toFixed(6)}` : 
                            'Unverified'}
                        </p>
                            </div>
                            </div>
